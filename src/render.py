...
def html(brief: dict) -> str:
    ...
    for c in cmap:
        ...
        cites = " ".join(f"<span class=cite>{_esc(x)}</span>" for x in c.get("citations", [])) or "<span class=nocite>no citation — unsupported</span>"
        cracks_html += f"""
...
.crackhead{{display:flex;gap:12px;align-items:center;font-size:12px;margin-bottom:6px}}
...
<h1>Thesis Red-Team — {_esc(name)}</h1>
...
<div class=foot>
<b>Analysis, not advice.</b> This is analytical commentary on the robustness of a user-supplied thesis, grounded only in the cited public documents. It is not investment advice and contains no BUY / SELL / HOLD instruction. Outputs may be incomplete or wrong and must not be relied upon for financial decisions.
</div>
</html>"""
